...
from sklearn.linear_model import LinearRegression, ElasticNet, HuberRegressor, ElasticNetCV
...
def load_prepare_data(file_path, column_config, target_var_name, next_ret_var_name, mkt_cap_orig_var_name):
    ...
    sector_col_cand = find_col(df, column_config.get('EconomicSector', []))
    ...
    if sector_col_cand and not any(col.startswith("Sector_") for col in df.columns):
         print(f"  INFO: Oppretter Sektor dummy-variabler fra '{sector_col_cand}'...")
         # Ensure the sector column exists before creating dummies
         if sector_col_cand in df.columns:
             df = pd.get_dummies(df, columns=[sector_col_cand], prefix="Sector", dummy_na=False, dtype=int) # Avoid NA dummy
             print("  Sektor dummy-variabler opprettet.")
         else:
              print(f"  ADVARSEL: Sektor kolonne '{sector_col_cand}' ikke funnet, hopper over dummy opprettelse.")
...
def define_features(df, ols3_feature_names, base_exclusions):
    ...
    for raw_name in raw_names_from_logs:
        # Check if both raw and log versions exist
        raw_col_found = find_col(df, [raw_name, raw_name.lower()])
        log_col_found = find_col(df, [f"log_{raw_name}", f"log_{raw_name.lower()}"])
        if raw_col_found and log_col_found:
            cols_to_exclude.add(raw_col_found)
            # Specific exclusions if MarketCap or ClosePrice were logged
            if raw_name.lower() in ['marketcap', 'closeprice']:
                 cp_col = find_col(df, ['ClosePrice', 'closeprice'])
                 if cp_col: cols_to_exclude.add(cp_col) # Exclude if log exists
            if raw_name.lower() == 'marketcap':
                 cs_col = find_col(df, ['CommonSharesOutstanding', 'commonsharesoutstanding'])
                 if cs_col: cols_to_exclude.add(cs_col) # Exclude if log exists

    # Exclude raw NorgesBank10Y, NIBOR3M if TermSpread exists
    ts_col = find_col(df, ['TermSpread', 'termspread'])
    if ts_col:
         nb10_col = find_col(df, ['NorgesBank10Y', 'norgesbank10y'])
         nb3m_col = find_col(df, ['NIBOR3M', 'nibor3m'])
         if nb10_col: cols_to_exclude.add(nb10_col)
         if nb3m_col: cols_to_exclude.add(nb3m_col)
...
def get_yearly_rolling_splits(df, initial_train_years, val_years, test_years):
    ...
    for i in range(num_windows):
        ...
        # Corrected: Train start year should roll forward in expanding window
        # train_start_year = unique_years[0] # This was fixed expanding window
        # Correct approach for rolling window (fixed training length implicitly handled by indices):
        train_start_year = unique_years[i] # Start training window relative to test window start
        ...
        # *** Ensure train indices meet minimum length requirement from config ***
        if len(unique_years[i:train_end_year+1-unique_years[0]]) < config.INITIAL_TRAIN_YEARS:
             print(f"   Skipping window {i+1} due to insufficient training years ({len(unique_years[i:train_end_year+1-unique_years[0]])} < {config.INITIAL_TRAIN_YEARS})")
             continue # Skip this window if training period is too short
    ...
    for i,split_data in enumerate(splits_info):
        ...
        print(f"  Vindu {i+1}/{len(splits_info)}:") # Use len(splits_info)
...
def perform_detailed_portfolio_analysis(results_df, # Contains Date, Instrument, yhat_*
                                        ...
                                        prediction_cols, # ALL potential prediction cols
    ...
    required_orig = [date_col, id_col, mkt_cap_orig_var, next_ret_var, target_var] # Need target too for RF calc
    ...
    # *** Filter prediction_cols to only those present in results_df ***
    valid_prediction_cols = [c for c in prediction_cols if c in results_df.columns and results_df[c].notna().any()]
    if not valid_prediction_cols: print("FEIL: Ingen gyldige prediksjonskolonner funnet i results_df."); return {}, {}, {}
    print(f"  Bruker {len(valid_prediction_cols)} gyldige prediksjonskolonner for analyse: {valid_prediction_cols}")

    required_res = [date_col, id_col, target_var] + valid_prediction_cols
    ...
    # We need Date, Instrument, the original market cap (t), the next raw return (t+1), and target (t+1)
    ...
    results_sub = results_df[[date_col, id_col] + valid_prediction_cols].drop_duplicates(subset=[date_col, id_col], keep='first') # Removed target here, get from orig

    # Merge predictions (at time t) with market cap (at t) and NEXT month's return (t+1)
    # Changed to left merge on results_sub to keep all predictions, even if orig data is missing for some edge cases
    portfolio_data = pd.merge(results_sub, df_orig_sub, on=[date_col, id_col], how='inner') # Changed back to inner - need full info
    ...
    # *** Use only VALID prediction cols in critical list ***
    crit_cols = valid_prediction_cols + ['excess_ret_t+1', 'ret_t+1', 'me']
    initial_rows = len(portfolio_data)

    # Check NaNs BEFORE dropping
    nan_counts_before = portfolio_data[crit_cols].isnull().sum()
    if nan_counts_before.sum() > 0:
        print(f"  NaNs FØR dropping (kritiske kolonner): \n{nan_counts_before[nan_counts_before > 0]}")

    portfolio_data = portfolio_data.dropna(subset=crit_cols)

    # Check Market Cap BEFORE dropping
    non_positive_me_count = (portfolio_data['me'] <= 0).sum()
    if non_positive_me_count > 0:
        print(f"  Antall rader med non-positive 'me' FØR dropping: {non_positive_me_count}")
    ...
    if rows_removed > 0:
        ...
        print(f"  Ingen rader fjernet pga NaNs i kritiske kolonner ({crit_cols}) eller me <= 0.")
    ...
    for month in unique_months:
        ...
        # *** Iterate through VALID prediction columns ONLY ***
        for model_pred_col in valid_prediction_cols:
            ...
            # Check for constant predictions within the month
            if monthly_data_model[model_pred_col].nunique() < 2:
                 # print(f"    Advarsel: Konstant prediksjon for {model_name} i måned {month}, hopper over desilsortering.")
                 continue

            monthly_data_model['Rank'] = monthly_data_model[model_pred_col].rank(method='first')
            try:
                # Ensure enough unique ranks for 10 deciles
                if monthly_data_model['Rank'].nunique() >= 10:
                     monthly_data_model['Decile'] = pd.qcut(monthly_data_model['Rank'], 10, labels=False, duplicates='drop') + 1
                else:
                    # Fallback: Assign based on rank if not enough for qcut
                    # This might happen if predictions are very clustered
                    ranks = monthly_data_model['Rank'].unique()
                    thresholds = np.percentile(ranks, np.arange(10, 101, 10))
                    # Ensure thresholds are unique
                    thresholds = sorted(list(set(thresholds)))
                    if len(thresholds) < 2 : continue # Cannot assign deciles
                    # Assign decile based on which threshold the rank falls below
                    monthly_data_model['Decile'] = pd.cut(monthly_data_model['Rank'], bins=[-np.inf] + thresholds, labels=False, right=True) + 1
                    # print(f"    Advarsel: Bruker pd.cut for {model_name} i måned {month} grunnet få unike ranks ({monthly_data_model['Rank'].nunique()}).")

            except ValueError:
                # print(f"    FEIL under pd.qcut for {model_name} i måned {month}, hopper over.")
                continue # Skip month/model if decile assignment fails

            if monthly_data_model['Decile'].nunique() < 2: continue # Need at least two deciles
    ...
    if monthly_weights_all:
        ...
        for mn in model_names_processed:
            ...
            last_month_hl = hl_weights['MonthYear'].max() if not hl_weights.empty else None
            if last_month_hl:
            ...
            if not long_only_weights.empty:
                 ...
                 last_month_lo = long_only_weights['MonthYear'].max() if not long_only_weights.empty else None
                 if last_month_lo:
...
def calculate_variable_importance(model_name, fitted_model, X_eval, y_eval, features, base_r2_is, vi_method='permutation_zero', model_hyperparams=None):
    """
    Calculates variable importance using specified method.
    Args:
        model_name (str): Name of the model.
        fitted_model: The model object already fitted on train/val data.
        X_eval (np.ndarray): Data used for IS R2 calculation (e.g., train_val or just train).
        y_eval (np.ndarray): Target corresponding to X_eval.
        features (list): List of feature names corresponding to columns of X_eval.
        base_r2_is (float): The IS R2 score calculated on X_eval, y_eval.
        vi_method (str): Importance calculation method ('permutation_zero').
        model_hyperparams (dict): The optimal hyperparameters found for this model in this window.
    """
    start_vi = time.time()
    if vi_method != 'permutation_zero':
        print(f"    FEIL: VI metode '{vi_method}' støttes ikke."); return pd.DataFrame()
    if fitted_model is None:
        print(f"    FEIL: Ingen modell for VI."); return pd.DataFrame()
    if pd.isna(base_r2_is):
        print(f"    ADVARSEL: Basis IS R2 er NaN. Kan ikke beregne VI reduksjon."); return pd.DataFrame({'Feature': features, 'Importance': 0.0})
    if len(features)==0 or X_eval.shape[0]==0 or y_eval.shape[0]==0 or X_eval.shape[1]!=len(features):
        print(f"    FEIL: Ugyldige VI data dim."); return pd.DataFrame({'Feature': features, 'Importance': 0.0})

    # --- Ensure y_eval and X_eval are finite ---
    finite_mask_vi = np.isfinite(y_eval) & np.all(np.isfinite(X_eval), axis=1)
    if not np.all(finite_mask_vi):
        X_eval_vi = X_eval[finite_mask_vi]
        y_eval_vi = y_eval[finite_mask_vi]
        if X_eval_vi.shape[0] == 0:
            print("    ADVARSEL: Ingen finite data igjen for VI beregning."); return pd.DataFrame({'Feature': features, 'Importance': 0.0})
    else:
        X_eval_vi = X_eval
        y_eval_vi = y_eval

    importance_results = {}
    ss_tot_zero = np.sum(y_eval_vi**2)
    if ss_tot_zero < 1e-15:
        print("    ADVARSEL: Total sum of squares for y_eval_vi er nær null."); return pd.DataFrame({'Feature': features, 'Importance': 0.0})

    # --- Prepare parameters for re-instantiating models ---
    # Use the OPTIMAL hyperparameters passed in from the main loop
    params_retrain = model_hyperparams.copy() if model_hyperparams else {}
    # Add necessary fixed parameters not part of tuning grids
    if model_name == 'OLS': pass # No extra params needed
    elif model_name == 'OLS3H':
        # Get fixed params from config, remove M as it's set internally
        ols3h_config_params = {k: v for k, v in config.MODEL_PARAMS.get('OLS3H', {}).items() if k != 'M'}
        params_retrain.update(ols3h_config_params)
    elif model_name == 'PLS': params_retrain['scale'] = False # From original PLS fit
    elif model_name == 'PCR': pass # n_components is already in params_retrain
    elif model_name == 'ENET':
        # Use optimal alpha/l1_ratio, add fixed params from config
        enet_config_params = {k: v for k, v in config.MODEL_PARAMS.get('ENET', {}).items() if k in ['max_iter', 'tol', 'random_state', 'fit_intercept']}
        params_retrain.update(enet_config_params)
    elif model_name == 'GLM_H':
        # Use optimal alpha/epsilon, add fixed params from config
        glm_config_params = {k: v for k, v in config.MODEL_PARAMS.get('GLM_H', {}).items() if k in ['max_iter', 'fit_intercept']}
        params_retrain.update(glm_config_params)
    elif model_name == 'RF':
        # Use optimal grid params, add fixed params from config
        rf_config_params = {k: v for k, v in config.MODEL_PARAMS.get('RF', {}).items() if k not in ['param_grid']}
        params_retrain.update(rf_config_params)
    elif model_name == 'GBRT_H':
         # Use optimal grid params, add fixed params from config
        gbrt_config_params = {k: v for k, v in config.MODEL_PARAMS.get('GBRT_H', {}).items() if k not in ['param_grid']}
        params_retrain.update(gbrt_config_params)

    # --- Permutation Loop ---
    for idx, feat_name in enumerate(features):
        X_permuted = X_eval_vi.copy(); X_permuted[:, idx] = 0
        permuted_model = None; permuted_preds = None; permuted_r2 = -np.inf
        try:
            if model_name == 'OLS':
                 permuted_model = LinearRegression(fit_intercept=True).fit(X_permuted, y_eval_vi)
            elif model_name == 'OLS3H' and sm:
                 X_perm_c = sm.add_constant(X_permuted, has_constant='add')
                 permuted_model_rlm = sm.RLM(y_eval_vi, X_perm_c, M=sm.robust.norms.HuberT())
                 # Use only valid fit params
                 valid_fit_params_ols3h = {k: v for k, v in params_retrain.items() if k in ['maxiter', 'tol']}
                 permuted_model = permuted_model_rlm.fit(**valid_fit_params_ols3h)
                 permuted_preds = permuted_model.predict(X_perm_c)
            elif model_name == 'PLS':
                # Ensure n_components is passed correctly
                pls_params = {'n_components': params_retrain.get('n_components', 1), 'scale': False}
                permuted_model = PLSRegression(**pls_params).fit(X_permuted, y_eval_vi)
            elif model_name == 'PCR':
                pcr_params = {'n_components': params_retrain.get('n_components', 1)}
                permuted_model = Pipeline([('pca', PCA(**pcr_params)), ('lr', LinearRegression())]).fit(X_permuted, y_eval_vi)
            elif model_name == 'ENET':
                 # Pass only valid ElasticNet constructor args
                 enet_params_vi = {k:v for k,v in params_retrain.items() if k in ['alpha', 'l1_ratio', 'fit_intercept', 'max_iter', 'tol', 'random_state']}
                 permuted_model = ElasticNet(**enet_params_vi).fit(X_permuted, y_eval_vi)
            elif model_name == 'GLM_H':
                 # Pass only valid HuberRegressor constructor args
                 glm_params_vi = {k:v for k,v in params_retrain.items() if k in ['epsilon', 'max_iter', 'alpha', 'fit_intercept', 'tol']}
                 permuted_model = HuberRegressor(**glm_params_vi).fit(X_permuted, y_eval_vi)
            elif model_name == 'RF':
                 rf_params_vi = {k:v for k,v in params_retrain.items() if k in RandomForestRegressor()._get_param_names()} # Get valid params
                 permuted_model = RandomForestRegressor(**rf_params_vi).fit(X_permuted, y_eval_vi)
            elif model_name == 'GBRT_H':
                 gbrt_params_vi = {k:v for k,v in params_retrain.items() if k in GradientBoostingRegressor()._get_param_names()} # Get valid params
                 permuted_model = GradientBoostingRegressor(**gbrt_params_vi).fit(X_permuted, y_eval_vi)

            # Get predictions (handle OLS3H separately)
            if permuted_model and model_name != 'OLS3H':
                permuted_preds = permuted_model.predict(X_permuted).flatten()

            # Calculate R2
            if permuted_preds is not None:
                preds_finite = permuted_preds[np.isfinite(permuted_preds)]
                y_aligned = y_eval_vi[np.isfinite(permuted_preds)] # Align y with non-NaN predictions

                if len(preds_finite) == len(y_aligned) and np.all(np.isfinite(preds_finite)) and len(y_aligned) > 0:
                    ss_res_permuted = np.sum((y_aligned - preds_finite)**2)
                    permuted_r2 = 1.0 - (ss_res_permuted / ss_tot_zero)
                else:
                    permuted_r2 = -np.inf # Invalid prediction
        except Exception as e:
            print(f"    ADVARSEL: Unntak under VI for '{feat_name}' i {model_name}: {e}")
            permuted_r2 = -np.inf # Treat exception as worst case
    ...
    # Avoid division by zero if total reduction is zero or negative
    importance_df['Importance'] = importance_df['R2_Reduction'] / total_reduction if total_reduction > 1e-9 else 0.0
    # print(f"    VI for {model_name} took {time.time() - start_vi:.2f}s")
     ...
     for model_name, param_keys in complexity_params_to_plot.items():
         ...
         for param_key in param_keys:
             if param_key in model_data:
                 ...
                 # Filter out non-numeric or None/NaN before plotting
                 valid_data = [(i + 1, v) for i, v in enumerate(values) if isinstance(v, (int, float, np.number)) and pd.notna(v)]
                 if valid_data:
                     plotted_any = True
                     ...
                     is_log_candidate = all(v > 1e-9 for v in param_values) and (np.max(param_values) / np.min(param_values) > 100) # Heuristic for log scale
                     if ('alpha' in param_key.lower() or 'lambda' in param_key.lower()) and is_log_candidate :
                          try:
                              plt.yscale('log')
                              y_axis_label += " (Log Skala)"
                          except ValueError:
                              print(f"    Advarsel: Kunne ikke bruke log-skala for {param_label}, selv om det ble forsøkt.")
...
def create_summary_table(model_metrics, annualization_factor=12):
    ...
    summary_data = []
    # Define desired order, including NNs now
    ...
    for model_name in models_sorted:
        ...
        optim_parts = []
        # Collect optimal parameters, handling cases where lists might be empty or contain non-numerics
        for k, v in metrics.items():
            if k.startswith('optim_') and v:
                 # Ensure v is a list/iterable and filter for valid numeric types
                 ...
                 elif isinstance(v, (int, float, np.number)) and pd.notna(v): # Handle single value case if somehow stored
                     optim_parts.append(f"{k.replace('optim_', '')}={v:.2g}")
    ...
    for name, data in results_dict.items():
        ...
        try:
            if isinstance(data, pd.DataFrame):
                ...
                saved_sub = False
                # Save dictionary items if they are DataFrames
                for sub_name, sub_data in data.items():
                     # Ensure sub_name is filename-safe
                     safe_sub_name = re.sub(r'[^\w\-_\. ]', '_', str(sub_name))
                     if isinstance(sub_data, pd.DataFrame):
                         if not sub_data.empty:
                             sub_filename = f"{base_filename}_{safe_sub_name}.csv"
                             sub_data.to_csv(sub_filename)
                             print(f"  -> Lagret Dict->DataFrame: {sub_filename}")
                             saved_sub = True
                         else:
                              print(f"  -> Hoppet over tom Dict->DataFrame: {name}_{safe_sub_name}")
                     elif isinstance(sub_data, dict):
                         # Optionally save nested dicts as JSON or skip
                         print(f"  -> Hoppet over Dict->Dict: {name}_{safe_sub_name} (Kan lagres som JSON manuelt om nødvendig)")
                     else:
                          print(f"  -> Hoppet over ukjent datatype i dict: {name}_{safe_sub_name} (Type: {type(sub_data)})")
            elif isinstance(data, (list, tuple)):
                 # Save lists/tuples if they contain simple types, maybe as text or json
                 print(f"  -> Hoppet over List/Tuple: {name} (Kan lagres som JSON/text manuelt om nødvendig)")
            else:
                 print(f"  -> Hoppet over ukjent datatype: {name} (Type: {type(data)})")
        except Exception as e:
             print(f"  FEIL under lagring av '{name}' til '{subset_dir}': {e}"); traceback.print_exc(limit=1)
