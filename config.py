...
OUTPUT_DIR = "ML_Pipeline_Results_Yearly_Percentile_Preprocessed_v2" # Changed output dir name slightly
...
COLUMN_CONFIG = {
    ...
    'EconomicSector': ['EconomicSector', 'economicsector'] # Added lowercase option just in case
}
...
# Check your PREPROCESSED file for exact names (case-sensitive!)
OLS3_FEATURE_NAMES = ["log_BM", "Momentum_12M", "log_MarketCap"] # Assumes these exact names exist
...
# --- Model Hyperparameters (Check consistency with paper/code) ---
# Note: The paper uses ξ for Huber, sklearn uses epsilon.
# Note: The paper uses ρ for ENet l1_ratio penalty mix (0=Lasso, 1=Ridge), sklearn uses l1_ratio (0=Ridge, 1=Lasso)
# The config below uses sklearn conventions.
MODEL_PARAMS = {
    'OLS': {},
    'OLS3H': {'maxiter': 100, 'tol': 1e-6}, # M is set internally in the train function
    'PLS': {'n_components_grid': [1, 3, 5, 8, 10, 15]}, # K in paper
    'PCR': {'n_components_grid': [1, 5, 10, 15, 20, 25]}, # K in paper
    'ENET': {'alphas': np.logspace(-6, 1, 8), 'l1_ratio': [0.1, 0.5, 0.9, 0.99, 1.0], 'cv_folds': 3, 'max_iter': 1000, 'tol': 0.001, 'n_jobs': -1}, # λ=alpha, ρ=l1_ratio (inverted), uses l2 loss
    'GLM_H': {'param_grid': {'alpha': [0.0001, 0.001, 0.01, 0.1, 1.0], 'epsilon': [1.1, 1.35, 1.5, 2.0]}, 'max_iter': 300}, # Uses Huber loss, alpha here is regularization strength, epsilon is Huber param ξ
    'RF': {'param_grid': {'n_estimators': [100], 'max_depth': [3, 6, 10], 'min_samples_leaf': [50, 100], 'max_features': ['sqrt', 0.33]}, 'n_jobs': -1, 'random_state': 42}, # B=n_estimators, L=max_depth
    'GBRT_H': {'param_grid': {'n_estimators': [100], 'learning_rate': [0.1], 'max_depth': [3, 5], 'min_samples_leaf': [50, 100], 'alpha': [0.9]}, 'loss': 'huber', 'random_state': 42}, # B=n_estimators, L=max_depth, ν=learning_rate, alpha is Huber quantile
    'NN_SHARED': {'param_grid': {'lambda1': [1e-5, 1e-4, 1e-3], 'learning_rate': [0.001, 0.01]}, 'epochs': 100, 'batch_size': 10000, 'patience': 5, 'ensemble_size': 10, 'random_seed_base': 42}, # lambda1 is L1 penalty
...
FILTER_SMALL_CAPS_PORTFOLIO = False # Keep as False unless specifically needed
ANNUALIZATION_FACTOR = 12

# --- Variable Importance ---
CALCULATE_VI = True
VI_METHOD = 'permutation_zero' # As per paper discussion
VI_PLOT_TOP_N = 20
MODEL_VI_STRATEGY = { # Reflecting capabilities and computation time
    'OLS': 'per_window', 'OLS3H': 'per_window', 'PLS': 'per_window',
    'PCR': 'per_window', 'ENET': 'per_window', 'GLM_H': 'per_window', # Linear models are fast enough
    'RF': 'last_window', 'GBRT_H': 'last_window', # Trees can be slower
    'NN1': 'last_window', 'NN2': 'last_window', 'NN3': 'last_window', # NNs definitely slower
...
COMPLEXITY_PARAMS_TO_PLOT = {
    ...
    'RF': ['optim_max_depth', 'optim_min_samples_leaf', 'optim_max_features'], # Added more RF params
    'GBRT_H': ['optim_n_estimators', 'optim_learning_rate', 'optim_max_depth', 'optim_min_samples_leaf'], # Added more GBRT params
...
# --- Seeds ---
GENERAL_SEED = 42
TF_SEED = 42 # Keep separate TF seed
